...
pub struct NetworkProperties {
    #[doc = "A value to describe how the outbound dependencies of a HDInsight cluster are managed. 'Managed' means that the outbound dependencies are managed by the HDInsight service. 'External' means that the outbound dependencies are managed by a customer specific solution."]
    #[serde(rename = "outboundDependenciesManagedType", default, skip_serializing_if = "Option::is_none")]
    pub outbound_dependencies_managed_type: Option<network_properties::OutboundDependenciesManagedType>,
...
pub mod network_properties {
    ...
    #[doc = "A value to describe how the outbound dependencies of a HDInsight cluster are managed. 'Managed' means that the outbound dependencies are managed by the HDInsight service. 'External' means that the outbound dependencies are managed by a customer specific solution."]
    #[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
    #[serde(remote = "OutboundDependenciesManagedType")]
    pub enum OutboundDependenciesManagedType {
        Managed,
        External,
        #[serde(skip_deserializing)]
        UnknownValue(String),
    }
    impl FromStr for OutboundDependenciesManagedType {
        type Err = value::Error;
        fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
            Self::deserialize(s.into_deserializer())
        }
    }
    impl<'de> Deserialize<'de> for OutboundDependenciesManagedType {
        fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
        where
            D: Deserializer<'de>,
        {
    ...
    impl Serialize for OutboundDependenciesManagedType {
        fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            match self {
                Self::Managed => serializer.serialize_unit_variant("OutboundDependenciesManagedType", 0u32, "Managed"),
                Self::External => serializer.serialize_unit_variant("OutboundDependenciesManagedType", 1u32, "External"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
